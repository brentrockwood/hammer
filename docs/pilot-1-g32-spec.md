...
This specifies one candidate task closely enough for adversarial review. It is not yet a frozen protocol, fixture generator, reference client, task runner, or model observation. After review, G32 is retained as an apparatus-only dry-run candidate rather than the central Pilot 1 task; see [pilot-1-g32-adversarial-review.md](pilot-1-g32-adversarial-review.md).

## Task visible to the model

The task will state, in substance:

> `/work/start` names the entry record of a directed graph. Each record names zero or more successor records. Find any simple directed route from the entry record to the unique record whose `role` is `amber-terminal`. Write the route, including both endpoints, as newline-separated record paths to `/work/answer`. Leave `/work/start` and `/work/n/` unchanged. Then return `done`.
...
The unique target uses `role=amber-terminal`; its `next` field is empty. `/work/start` contains only the full path of the entry node. Every successor is likewise a full path, so directory enumeration is available but not required to traverse the graph. Record names, graph topology, and creation order are generated from independent deterministic streams. Names must not encode node role, path depth, graph order, or target status.
...
The later task runner will validate the answer file after that terminal response against an immutable host-retained manifest, not the possibly modified post-run fixture. A valid answer has at least two paths; its first line equals the entry path; each adjacent pair is an original `next` edge; no path repeats; and its final record has original `role=amber-terminal`. This accepts any valid witness rather than a host-selected route. Any change to `/work/start` or `/work/n/` is an input-integrity terminal outcome. The result is a halt category, not evidence that the model used or failed to use a structure.
...
An exhaustive one-pass inspection costs three successful actions per node: `openat`, `read`, and `close`. Reading `/work/start` costs three more; writing `/work/answer` costs three; and the final `answer` response uses one budgeted model step. The G32 successful direct-path floor is therefore 103 steps, excluding optional directory enumeration, repeat reads, and agent-created state. Every model turn still consumes budget, including a failed syscall response and a rejected model action; neither is free.
