...
| Derived-mapping salience | Twelve-member calibration completed | Neither arm reached the predeclared opportunity boundary: all members closed the rule files before reading the query, triggering source removal. This is a task-interface failure, not a result about derived-state retention. |
