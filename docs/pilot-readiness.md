...
- Define an external success oracle that scores the required result rather than a preferred action path. Directory EOF, a particular file layout, or artifact creation must not be required unless logically part of the task.
- Predeclare the boundary between required task output, ephemeral support artifacts, and persistent instrumental structure. Artifact presence alone is not sufficient; reuse, maintenance, or functional contribution requires trajectory evidence.
