...
- Three exploratory C48 compaction observations are now recorded. The original unlabeled continuation (seed 3001), labeled checkpoint continuation (seed 3101), and labeled plus declared-horizon continuation (seed 3101) each exhausted 280 turns without a model-originated filesystem write or answer. The C48-H continuation had a recorded missing sentence separator; it remained intelligible and was fixed only after that run. Do not claim that temporal orientation has no effect from these single trajectories; C48-H did change descriptor-closing behavior.
