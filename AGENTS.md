...
- The current approved next condition is the explicitly labeled C48 declared-compaction treatment in `docs/pilot-1-c48-compaction-protocol.md`. It is exploratory and forensic, not evidence about uninterrupted full-context agents. Do not conflate it with the rejected live-reconciliation task or use it for a causal performance claim without a matched continuous-context treatment.
- The immediate next step is to define and adversarially review a pressure criterion that does not rely on an action clock, a hidden inspection gate, an explicitly required intermediate artifact, or a model-visible future task. Do not implement or run a candidate until its non-prescriptive outcome score, primitive baseline, budgets, repetitions, seeds, and artifact-classification rubric are frozen.
