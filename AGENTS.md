...
- The C48 append-affordance H0 pair is complete. `append_calibration.py` passed create → close → reopen → append under the fixed isolation boundary. With seed 3301, the no-append control exactly replayed the earlier H0 model/action stream after run-specific metadata were removed. The append-visible member received the explicit grammar line and `/agent --append`, but never requested append, made no write, and left no model-created state. It made 139 opens, 137 reads, no closes, and four rejections, versus the control's 130 opens, 127 reads, 20 closes, and two rejections. This is one observed prompt/affordance-condition difference, not evidence that append semantics caused it or that append is generally useful. The pair is not Pilot 1 and does not repair the H12 answer-gating pathology.
- D96 is now frozen and has one unreplicated candidate observation. Its scripted primitive baseline passed in 297 actions. The first model run (`d96-pilot-candidate-20260820T004248Z`, seed 3401) enumerated the directory once, read 48 distinct records, wrote only `/work/answer`, and submitted `done` after 104 model calls with 376 turns unused. The answer had 48 rather than 96 lines, included one malformed path and one duplicate, and failed validation. It requested no append, created no support state, and reused no model-created state. The ordinary terminal channel accepted the premature answer, so this is a task-level failure rather than a C48-style answer-gating artifact. It is not a general Pilot 1 result; do not increase size, add a terminal gate, or otherwise repair the task around this single trajectory without a separate decision.
