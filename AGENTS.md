...
- The next frozen pilot candidate is D96, a 96-record dependency-ordering task in one continuous model context. Its output oracle accepts any valid topological order; no checkpoint, support artifact, or append use is required. Run `d96.py --scripted`, inspect and publish that baseline, then run `d96.py` with the exact frozen settings in `docs/pilot-d96-dependency-ordering-protocol.md`. The first model run is an unreplicated candidate observation, not a general Pilot 1 result.
