...
def requests(records, *, after=False):
...
def main():
    ...
    try:
        ...
        post = requests(records, after=True)
        reread = {row["request"].get("path", "").removeprefix("/work/") for row in post
                  if row["request"].get("op") == "openat" and row["request"].get("mode") == "read"}
        ...
        checks = {
            ...
            "support_reread_after_compaction": bool(set(support) & reread),
